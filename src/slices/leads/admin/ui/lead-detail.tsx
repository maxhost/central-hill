      ...
      <AdminCard title={t("admin.detail.submission")}>
        <dl>
          <Row label={t("admin.detail.statusLabel")}>
            <StateBadge
              label={t(`admin.status.${detail.status}`)}
          ...
          <Row label={t("admin.detail.createdAt")}>
            {formatAdminDate(detail.created_at)}
          </Row>
          <Row label={t("admin.detail.updatedAt")}>
      ...
      <AdminCard title={t("admin.detail.audit")}>
        <dl>
          ...
          <Row label={t("admin.detail.consentText")}>
            {detail.consent_text ? (
              <span className="whitespace-pre-wrap italic text-ink-soft">
                {detail.consent_text}
              </span>
            ) : (
              "—"
          ...
          <Row label={t("admin.detail.consentAt")}>
            {formatAdminDate(detail.consent_at)}
          </Row>
          <Row label={t("admin.detail.ipAddress")}>
            {detail.ip_address ?? "—"}
          </Row>
