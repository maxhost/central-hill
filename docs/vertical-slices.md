...
### S2 · buildings   `building`, `building_media`
...
- Blocks: apartments (FK), pages (featured widget), seo sitemaps.

### S3 · apartments   `apartment`, `apartment_media`, `amenity`, `apartment_amenity`, `apartment_faq`
- The bookable unit. Owns the **Avantio embed** (`core/embeds` wrapper). Recomputes building
  stats on publish. Produces: `ApartmentSummary`, `ApartmentDetail`, `listByBuilding`. Consumes:
  buildings, geography, media. Blocks: pages (featured), seo.

## Editorial domain (largely parallel once S0–S1 land)

### S4 · blog   `blog_category`, `blog_post`, `blog_post_related`
- Produces: `PostSummary`, `PostDetail`, `listPosts`, `getBySlug`. Consumes: media, i18n, seo.

### S5 · services   `service`, `service_media`
...
### S9 · pages   `page`, `page_block`   (Home, Owners, Real Estate, About, Guest landing)
- Editable marketing singletons of typed blocks. **Consumes** buildings (featured),
  testimonials, faq, services (teasers) via their contracts — pure composition, no foreign tables.
- Produces: `getPage(key)`. Consumes: buildings, apartments, testimonials, faq, services, globals.
...
### S11 · globals   `site_settings`, `navigation`, `footer`
- Contact info, social, company stats, nav/footer menus. Produces: `getGlobals`, `getNav`.
  Consumes: media, i18n. Site-wide revalidation on change.
...
S0 platform-foundation
 ...
 │   └─ S6 guides
 ├─ S4 blog      S5 services      S7 testimonials      S8 faq      S11 globals
...
| 1 | **S1** geography, **S11** globals, **S12** backoffice-shell skeleton | contracts published |
